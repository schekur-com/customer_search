...
class SCHEKURReportEngine:
    def __init__(self):
        ...
        self.output_html = "data_vault/index.html" # GitHub Pages için kök dizin çıktısı
        self.output_pdf = "data_vault/istihbarat_cikti_raporu.pdf"

    def generate_html_template(self, df_leads, metadata):
        """Web arayüzü ve PDF için kullanılacak dinamik HTML şablonunu oluşturur."""
        
        # Tablo satırlarını dinamik olarak inşa etme
        table_rows = ""
        for idx, row in df_leads.iterrows():
            ...
            <tr>
                ...
                <td>{row.get('address', 'Belirtilmemiş')}</td>
                <td>{row.get('matched_product', 'Genel')}</td>
                <td style="text-align: center; font-weight: bold;">{row.get('verified_hs_code', '-')}</td>
                <td style="text-align: center; font-weight: bold; color: #2563eb;">{row.get('total_shipments_12months', 0)}</td>
                <td><a href="{row['website']}" target="_blank" class="link-text">{row['website'].replace('https://','').replace('www.','')}</a><br><small>{row['phone']}</small></td>
                <td><span class="{score_class}">{int(row['final_export_score'])} / 100</span></td>
            </tr>
            """
        ...
        <head>
            ...
            <style>
                * {{ box-sizing: border-box; }}
                ...
                body {{ font-family: 'Segoe UI', Arial, sans-serif; color: #1e293b; margin: 0; padding: 0; background-color: #f8fafc; font-size: 10pt; line-height: 1.3; }}
                ...
                .btn {{ background-color: #16a34a; color: white; padding: 6px 12px; border-radius: 4px; text-decoration: none; font-weight: bold; font-size: 9pt; }}
        ...
        <body>
            <div class="header-container">
                <div class="header-grid">
                    <div class="header-cell">
                        ...
                        <div class="title-sub">Dinamik İhracat Müşteri Hedefleme ve Konşimento Eşleştirme Paneli</div>
                    </div>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="width: 22%;">Adres / Konum</th>
                        <th style="width: 14%;">Eşleşen Kriter</th>
                        <th style="width: 8%;">GTİP Kodu</th>
                        <th style="width: 8%;">12A Konşimento</th>
                        <th style="width: 16%;">İletişim & Web</th>
            ...
            <div class="footer">SCHEKUR-2026 Altyapısı ve Güvenli Veri İşleme Protokolü ile şifreli olarak üretilmiştir.</div>
        </body>
        </html>
        """
        return html_content
    ...
    def build_reports(self, current_hs, current_desc, current_kw):
        """Ham veriyi okur, şablonla birleştirir ve dosyaları basar."""
        ...
        with open(self.output_html, "w", encoding="utf-8") as f:
            f.write(html_data)
        print("[BAŞARILI] Interaktif HTML Arayüzü 'data_vault/index.html' olarak güncellendi.")

        # weasyprint ile PDF Çıktısını Hazırla
        # PDF dosyasında indirme butonlarının görünmemesi için basit bir CSS manipülasyonu ekliyoruz
        ...
        print("[BAŞARILI] Ofis/Yönetim için kurumsal PDF belgesi 'data_vault/istihbarat_cikti_raporu.pdf' olarak basıldı.")

if __name__ == "__main__":
    # Pipeline parametrelerini dinamik olarak yakalama
    HS = os.getenv("DYNAMIC_HS_CODE", "8481.80")
    DESC = os.getenv("DYNAMIC_PRODUCT_DESC", "Industrial Valves / Vana")
    KW = os.getenv("TARGET_KEYWORDS", "industrial flow control distributor")
